...
This script provides a universal executor for running Playwright automation code from various sources.
...
- **Automatic Dependency Management**: Installs Playwright if missing
- **Smart Code Wrapping**: Automatically wraps code in async IIFE with proper imports
- **Temporary File Management**: Creates temp execution files with cleanup
- **Environment Headers**: Supports extra HTTP headers from environment variables
...
# Execute inline code
node run.js 'await page.goto("https://example.com")'
...
- `PW_HEADER_NAME` / `PW_HEADER_VALUE`: Single header pair
