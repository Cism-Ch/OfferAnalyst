...
This module provides reusable utility functions for Playwright automation tasks.

## Features

### Core Browser Management
- **Browser Launch**: Support for Chromium, Firefox, WebKit with configurable options
- **Context Creation**: Smart context creation with environment headers
- **Page Management**: Viewport setup, user agent configuration

### Automation Utilities
- **Smart Waiting**: Page load and element wait strategies
- **Safe Interactions**: Retry logic for clicks and typing
- **Navigation**: Scrolling, authentication, cookie banner handling
- **Data Extraction**: Text extraction, table data parsing
- **Screenshots**: Timestamped screenshot capture

### Advanced Features
- **Environment Headers**: Parse headers from environment variables
- **Dev Server Detection**: Auto-detect running development servers
...
PW_HEADER_NAME="Authorization"
PW_HEADER_VALUE="Bearer token"
# OR
PW_EXTRA_HEADERS='{"Authorization": "Bearer token", "X-Custom": "value"}'
...
- `launchBrowser()`: Launch browser with default options
- `createContext()`: Create browser context with headers
- `createPage()`: Create page with viewport settings
- `safeClick()`: Click with retry logic
- `safeType()`: Type with validation and options
- `detectDevServers()`: Find running development servers
- `getExtraHeadersFromEnv()`: Parse headers from environment
