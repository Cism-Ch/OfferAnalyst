  ...
  const operation = async () => {
    const response = await openrouter.chat.send({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Analyze these ${simplifiedOffers.length} offers and rank top ${limit}: ${JSON.stringify(simplifiedOffers)}`,
        },
      ],
    ...
    console.log(
      `Analyze (Gemini): AI response received (${text.length} chars)`,
    );
    ...
    console.log(
      `Analyze: Successfully analyzed ${mergedTopOffers.length} offers`,
    );
