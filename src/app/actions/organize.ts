  ...
  const operation = async () => {
    const response = await openrouter.chat.send({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Organize these ${simplifiedOffers.length} offers: ${JSON.stringify(simplifiedOffers)}`,
        },
      ],
    ...
    console.log(
      `Organize (Gemini): AI response received (${text.length} chars)`,
    );
    ...
    console.log(
      `Organize: Successfully organized into ${hydratedCategories.length} categories`,
    );
